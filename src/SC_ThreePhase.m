...
function [U_T,I_T,U_P,I_P,ScnodeCon] = SC_ThreePhase(Z1,ScNode,UfBase,...
                                                    Transform120ToABC,BranchNumber,...
...
%% 短路节点计算（根据边界条件）
    ...
    If_P = Transform120ToABC * If_T;%将正序、负序、零序电流转换为相分量，三行一列
    ...
    Uf_P= Transform120ToABC * Uf_T;%将正序、负序、零序电压转换为相分量，三行一列
...
%%各节点支路计算
    ...
    U_P= Transform120ToABC * U_T;%将正序、负序、零序电压转换为相分量，每列为一个节点的三相分量
    ...
    I_P = Transform120ToABC * I_T; % 将正序、负序、零序电流转换为相分量
