...
function [U_T,I_T,U_P,I_P] = SC_SinglePhase(Z1,Z2,Z0,S,ScNode,UfBase,Transfrom120ToABC,NodeNumbers,BranchNumber,BranchStartNode,BranchEndNode)
...
%% 短路节点计算（根据边界条件）
    %电流
    If1 = UfBase / (Z1(ScNode, ScNode) + Z2(ScNode, ScNode) + Z0(ScNode, ScNode)); %正序电流计算
    If2 = If1; %单相短路电流的边界条件
    If0 = If1;
